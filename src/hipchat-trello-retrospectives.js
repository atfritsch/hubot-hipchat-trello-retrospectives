...
#									within a specified Trello organization.  If the board doesn't exist,
#									the board is created and assigned lists representing commonly used
      ...
      checkIfBoardExists(room, function(boardId) {
        if(boardId !== false) {
          createCard(cardText, boardId, function(card) {
            msg.send("Successfully added retrospective card: " + card.shortUrl);
          });
        } else {
...
//***** HELPER FUNCTIONS *****
  ...
  function removeDefaultLists(boardId, callback) {
    t.get("/1/boards/" + boardId + "/lists", function(listsErr, lists) {
      ...
      async.eachSeries(lists, function(list, subcallback) {
        t.put("/1/lists/" + list.id + "/closed", { value: true }, function(listClosedErr) {
  ...
  function addRetroLists(boardId, columns, callback) {
    async.eachSeries(columns, function(column, subcallback) {
      t.post("/1/lists/", {name: column, idBoard: boardId, pos: 'bottom' }, function(newListErr, newList) {
  ...
  function checkIfBoardExists(roomName, callback) {
    t.get("/1/organizations/" + process.env.TRELLO_ORGANIZATION + "/boards", { filter: "open" }, function(err, data) {
      if (err) throw err;
  ...
  function createCard(cardText, boardId, callback) {
    t.get("1/boards/" + boardId + "/lists", { filter: "open" }, function(err, lists) {
      if (err) throw err;

      if(lists.length) {
        t.post("1/cards/", {due: null, idList: lists[0].id, urlSource: null, name: cardText}, function(cardErr, card) {
